...
1. At your DNS provider, create/update:
   - `afromiaam.fr`     → A → `<VPS_IP>`
   - `www.afromiaam.fr` → A → `<VPS_IP>`
2. Set a **low TTL** (e.g. 300s) first so you can roll back quickly if needed.
3. Wait for propagation (`dig +short afromiaam.fr`), then watch Caddy obtain certs:
...
- **Authorized JavaScript origins:** `https://afromiaam.fr`
- **Authorized redirect URI:** `https://afromiaam.fr/api/auth/oauth/google/callback`
...
- **Domains:** `afromiaam.fr` (and `www.afromiaam.fr`)
...
curl -fsS https://afromiaam.fr/api/health

# Security headers present (HSTS, nosniff, frame-deny, no Server header).
curl -sI https://afromiaam.fr | grep -iE 'strict-transport|x-content-type|x-frame|referrer-policy|^server'

# Uploaded image served directly by Caddy with a long cache.
curl -sI https://afromiaam.fr/uploads/<some-image>.jpg | grep -i cache-control
```

Functional smoke test (manual, in a browser):

- [ ] `https://afromiaam.fr` loads with a valid certificate; `www` redirects/works.
