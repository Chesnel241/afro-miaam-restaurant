...
export function HowItWorks({ withCta = true }: { withCta?: boolean }) {
  return (
    <section className="bg-cream py-14 sm:py-20" id="comment-ca-marche">
      <div className="container-x">
        <div className="grid gap-10 md:grid-cols-[1.5fr_1fr] md:items-start">
              ...
              {STEPS.map((step) => (
                <li key={step.number} className="relative flex flex-col items-start">
                  <div className="relative">
                    <span className="inline-flex h-14 w-14 items-center justify-center rounded-full bg-creamSoft text-primary shadow-card sm:h-16 sm:w-16">
                      {step.icon}
                    </span>
                  ...
                  <p className="mt-3 text-xs font-bold uppercase tracking-widest text-primary sm:mt-4">
                    {step.title}
                  </p>
                  <p className="mt-1 text-sm text-primary/75 sm:text-base">{step.sub}</p>
                </li>
              ))}
            </ol>
          </div>
          ...
          <div className="relative overflow-hidden rounded-2xl bg-primary bg-grain p-6 text-cream shadow-soft sm:p-7">
            <div className="afro-side-pattern absolute inset-0 opacity-50" aria-hidden="true" />
            <div className="relative">
              <span className="inline-flex h-12 w-12 items-center justify-center rounded-full bg-cream/10 text-cream sm:h-14 sm:w-14">
